...
function my_calendar_filter( $output ) {
	...
	<caption>' . sprintf($calendar_caption, $wp_locale->get_month($thismonth)) . '</caption>
	<thead>
	<tr>';
	$myweek = array();
	...
	for ( $day = 1; $day <= $daysinmonth; ++$day ) {
		...
		if ( in_array($day, $daywithpost) ) // any posts today?
				$calendar_output .= '<a href="/' . intval($thismonth) . "-" . intval($day) . '" title="' . esc_attr( $ak_titles_for_day[ $day ] ) . "\">$day</a>";
		else
			$calendar_output .= $day;
		$calendar_output .= '</td>';
